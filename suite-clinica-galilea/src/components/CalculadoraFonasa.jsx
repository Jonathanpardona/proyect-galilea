      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: 940, margin: '0 auto', padding: '32px 24px' }}>
        <div style={{ marginBottom: 28 }}>
          <span style={{ display: 'inline-block', fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--chip)', background: 'var(--chipbg)', border: '1px solid var(--chipbd)', borderRadius: 99, padding: '3px 12px', marginBottom: 12 }}>
            FONASA · COPAGO POR TRAMO
          </span>
          <h1 style={{ fontSize: 32, fontWeight: 800, color: 'var(--tx1)', letterSpacing: '-0.03em', lineHeight: 1.15, marginBottom: 6 }}>
            Calculadora FONASA
          </h1>
          <p style={{ fontSize: 14, color: 'var(--tx2)' }}>Cálculo de copago por tramo para prestaciones de laboratorio</p>
        </div>

        <div className="tramo-selector" style={{ marginBottom: 20 }}>
          {TRAMOS.map(t => (
            <button key={t.tramo} className={`btn-tramo ${tramo === t.tramo ? 'activo' : ''}`} onClick={() => setTramo(t.tramo)}>
        ...
        <div className="modulo">
          <div className="grid-2">
            <div>
              ...
              {PRESTACIONES.map(p => (
                <label key={p.codigo} className={`checklist-item ${seleccion[p.codigo] ? 'checked' : ''}`} style={{ justifyContent: 'space-between' }}>
                  <input type="checkbox" checked={!!seleccion[p.codigo]} onChange={() => toggle(p.codigo)} />
              ...
              {seleccionadas.length === 0 ? (
                ...
                <>
                  ...
                  <div className="resumen-fonasa">
                    <div className="resumen-fila"><span>Total prestaciones</span><strong>{fmt(resumen.totalBase)}</strong></div>
                    <div className="resumen-fila verde"><span>Cubre FONASA (Tramo {tramo})</span><strong>{fmt(resumen.cubierto)}</strong></div>
                    <div className="resumen-fila naranja"><span>Copago paciente ({tramoActual.copago}%)</span><strong>{fmt(resumen.copagoPaciente)}</strong></div>
                  </div>
                </>
              )}
            </div>
